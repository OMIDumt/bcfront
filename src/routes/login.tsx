...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Sign In — Morshed" }] }),
  component: LoginPage,
});
    ...
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md bg-card rounded-3xl p-8 shadow-elegant border">
          <h1 className="text-2xl font-bold text-center">{t("auth.signin.title")}</h1>
          <p className="text-muted-foreground text-center text-sm mt-2 mb-6">{t("auth.signin.sub")}</p>
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            ...
            <div><Label>{t("auth.password")}</Label><Input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1" /></div>
            <Button type="submit" disabled={loading} className="w-full bg-deep text-cream hover:bg-deep/90 rounded-full h-11">
              {loading ? t("common.loading") : t("auth.signin.btn")}
            </Button>
          </form>
          <p className="text-center text-sm mt-6 text-muted-foreground">
            {t("auth.dont")} <Link to="/signup" className="text-primary font-semibold">{t("auth.signup.btn")}</Link>
          </p>
        </div>
      </div>
    </div>
